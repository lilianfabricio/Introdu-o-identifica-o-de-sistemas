close all;
% Carregando os dados
dados = load('dataBenchmark.mat');
entrada_val = dados.uVal; % teste
saida_val = dados.yVal;

entrada_est = dados.uEst; % treino
saida_est = dados.yEst;

% Normalizando os dados de entrada e saída
entrada_val = (entrada_val - min(entrada_val)) / (max(entrada_val) - min(entrada_val));
saida_val = (saida_val - min(saida_val)) / (max(saida_val) - min(saida_val));

entrada_est = (entrada_est - min(entrada_est)) / (max(entrada_est) - min(entrada_est));
saida_est = (saida_est - min(saida_est)) / (max(saida_est) - min(saida_est));

erro_depois = [];
saida_prevista = [];

% ordem do modelo SEMP
na = 4; % ordem de auto-regressão
nb = 4; % ordem de entrada
...
X = [];
for i = max(na,nb)+1:length(entrada_est)
    regressores = [saida_est(i-na:i-1).^2; entrada_est(i-nb:i-1).^2];
    X = [X; regressores'];
end
...
while ~isempty(Psi_out)
    ...
    while i <= length(Psi_out)
        ...
        b = regress(saida_est(max(na,nb)+1:end), X_sub);
        
        % Calculando a saída prevista para amostra i
        saida_prevista = X(:, Psi_out(i)) * b;
        
        % Calculando o erro
        erro = saida_est(max(na,nb)+1:end) - saida_prevista;
        ...
        var_saida = var(saida_est(max(na,nb)+1:end));
...
X_test = [];
for i = max(na,nb)+1:length(entrada_val)
    regressores_test = [saida_val(i-na:i-1).^2; entrada_val(i-nb:i-1).^2];
...
erro_test = saida_val(max(na,nb)+1:end) - saida_prevista_test;
...
figure;
plot(saida_est(max(na,nb)+1:end), 'Color', 'b');
hold on;
...
figure;
plot(saida_val(max(na,nb)+1:end), 'Color', 'b');
hold on;
